...
const UpgradeStorageDialog: React.FC<UpgradeStorageDialogProps> = ({ isOpen, onOpenChange }) => {
  const { user } = useAuth();
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            You are currently using {formatBytes(currentStorageUsed)} of {formatBytes(currentStorageLimit)}.
            Choose a plan below to increase your storage capacity.
          </DialogDescription>
        </DialogHeader>
          ...
          {storagePlans.map((plan) => (
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  <h4 className="font-semibold">{plan.name} - {plan.sizeGB} GB</h4>
                ...
                {plan.current ? (
                  ...
                  <Button 
                    onClick={() => alert(`Selected ${plan.name} plan. Payment integration coming soon!`)}
                  >
                    Choose Plan
                  </Button>
                )}
        ...
        <DialogFooter className="sm:justify-start">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            ...
            Plan selection is currently for informational purposes.
          </p>
          <DialogClose asChild>
            <Button type="button" variant="outline" className="mt-2 sm:mt-0 sm:ml-auto">
              Close
            </Button>
          </DialogClose>
        </DialogFooter>
