...
import React, { createContext, useState, useContext, useEffect, ReactNode, useCallback } from "react"; // Added useCallback
import { User } from "@/types";
import { authApi } from "@/services/api";
import { useToast } from "@/components/ui/use-toast";
import { decryptMasterKey, arrayBufferToBase64, base64ToArrayBuffer } from "@/lib/cryptoUtils"; // Added base64ToArrayBuffer
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [masterCryptoKey, setMasterCryptoKey] = useState<CryptoKey | null>(null); // This is the primary CryptoKey state now
  ...
  const getMasterCryptoKey = useCallback(async (): Promise<CryptoKey | null> => {
    if (masterCryptoKey) {
    ...
    try {
      ...
      if (storedRawKey) {
        ...
        if (storedEncryptedKey && userData) {
          console.log("Raw master key not in session, but encrypted key is available in localStorage. User may need to re-login to decrypt and use it.");
        }
      }
    } catch (error) {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (userData.encryptedMasterKey) {
        try {
          const decryptedKeyArrayBuffer = await decryptMasterKey(userData.encryptedMasterKey, password);
          const decryptedKeyBase64 = arrayBufferToBase64(decryptedKeyArrayBuffer);
          setRawMasterKey(decryptedKeyBase64);
          sessionStorage.setItem('rawMasterKey', decryptedKeyBase64);
          localStorage.setItem('encryptedMasterKey', userData.encryptedMasterKey);
          ...
          toast({
            ...
            description: "Could not decrypt your master key. Please check your password. Some features might be unavailable.",
            variant: "destructive",
          });
        }
      } else {
        console.warn("User does not have an encryptedMasterKey. Skipping decryption.");
